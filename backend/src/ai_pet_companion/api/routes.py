...
async def models_available(request: Request):
    ...
    or key text is ever exposed â€” only the boolean and the real model list.
    """
    ...
    async def probe(name: str) -> tuple[str, dict]:
        ...
        except Exception:  # noqa: BLE001 â€” timeout or anything else => unreachable
            models = None
...
async def set_keys(body: KeysRequest, request: Request):
    ...
    Returns only the names written and a fresh brain probe â€” never any value.
    """
    if not body.keys:
...
# â”€â”€ Spotify (OAuth connect, for the play_music / control_playback tools) â”€â”€
...
async def spotify_login(request: Request):
    """Begin the OAuth handshake â€” redirect the browser to Spotify's consent page."""
    config = request.app.state.config
    if not spotify.configured():
        raise HTTPException(
            400, "Spotify app credentials missing â€” set SPOTIFY_CLIENT_ID/SECRET in .env."
        )
...
# â”€â”€ Pet / Den â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _detect_brain(config) -> dict:
    ...
    try:
        ...
        ollama = False  # Ollama not running â€” expected, not an error
    cloud_keys = any(
...
async def get_vision(request: Request):
    ...
    off-device â€” so the UI can warn before capturing. Resolves the `vision`
...
async def list_skills(request: Request):
    """The companion's approved (active) skills â€” drives the village's earned monuments."""
...
class ChatRequest(BaseModel):
    ...
    # URL). Sent to the vision model for this turn only â€” never written to history.
