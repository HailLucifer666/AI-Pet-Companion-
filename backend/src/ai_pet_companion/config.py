...
DB_PATH = DATA_DIR / "ai_pet_companion.db"
...
class TrustConfig(BaseModel):
    ...
    # Named tools allowed to auto-run *regardless of tier* â€” least-privilege
...
class ActionsConfig(BaseModel):
    ...
    # default â€” never file:/javascript:/data:/custom protocol handlers.
...
def _config_path() -> Path:
    explicit = os.environ.get("ai_pet_companion_CONFIG")
    if explicit:
...
def write_env_keys(env_path: Path, keys: dict[str, str]) -> list[str]:
    """Merge secret keys into the .env file and live ``os.environ`` â€” no restart.
