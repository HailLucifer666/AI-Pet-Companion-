...
an active device â€” failures are mapped to clear, user-facing messages.
"""
...
# â”€â”€ Token store (single row, id=1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ OAuth handshake â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _token_set_from_payload(data: dict, *, fallback_refresh: str = "") -> TokenSet:
    # Refresh responses may omit refresh_token â€” keep the existing one.
    return TokenSet(
...
# â”€â”€ App token (client credentials) â€” public search, no user login/Premium â”€â”€
...
async def app_token() -> str:
    ...
    if resp.status_code != 200:
        raise SpotifyError("Spotify app credentials rejected â€” check SPOTIFY_CLIENT_ID/SECRET.")
    data = resp.json()
...
# â”€â”€ Authenticated client â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class SpotifyClient:
    ...
    async def _access_token(self) -> str:
        ...
        if toks is None:
            raise SpotifyError(
                "Spotify isn't connected yet â€” open Settings and click Connect Spotify."
            )
    ...
    async def _refresh(self, toks: TokenSet) -> TokenSet:
        ...
        if resp.status_code != 200:
            raise SpotifyError(
                "Spotify login expired â€” please reconnect in Settings."
            )
    ...
    async def _api(self, method: str, path: str, *, retry: bool = True, **kwargs) -> httpx.Response:
        ...
        if resp.status_code == 401 and retry:
            # Token rejected mid-flight â€” force a refresh and try once more.
            toks = await load_tokens(self.db)
            if toks:
    ...
    async def play_uri(self, uri: str) -> None:
        ...
        if resp.status_code == 404:  # no active device â€” pick one and retry
    ...
    def _raise_for_playback(resp: httpx.Response) -> None:
        ...
        raise SpotifyError("Spotify wouldn't accept that just now â€” try again in a moment.")
