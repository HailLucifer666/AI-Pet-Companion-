"""Pet XP engine â€” server-side only.

Light is conserved: XP is earned solely from real work (PRD Â§3.3), never from
affection or play. Every award is rate-limited per day per type (anti-farming),
and deleting a memory claws its light back. Stage is *monotonic* â€” clawing back
...
# event type -> XP (PRD Â§3.3). Failed tool calls / dedup'd memories never call award().
...
# Max positive awards per type per (UTC) day. Generous â€” anti-farming, not punitive.
...
def stage_for_xp(xp: int) -> int:
    """1..4 â€” the highest stage whose threshold is reached."""
    stage = 1
