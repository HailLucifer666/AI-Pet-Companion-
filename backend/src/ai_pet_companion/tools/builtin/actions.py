...
only when named in ``trust.auto_approve_tools`` â€” never via a blanket trust raise,
...
def spawn(target: str) -> None:
    """Launch a URI (browser/protocol handler) or an executable â€” fire-and-forget.
    ...
    if sys.platform == "win32":
        ...
        # handlers (e.g. ``spotify:``) and launches PATH apps â€” all detached.
