...
def wmo_category(code: int) -> Category:
    ...
    https://open-meteo.com/en/docs — codes: 0 clear; 1–3 cloud cover; 45/48 fog;
    51–67 drizzle/rain; 71–77 & 85/86 snow; 80–82 rain showers; 95–99 thunderstorm.
    """
    if code in (0, 1):
    ...
    if code in (95, 96, 99):
        return "storm"
    return "cloudy"  # unknown → safe, unobtrusive default
...
async def _resolve_location(client: httpx.AsyncClient) -> dict | None:
    ...
    except Exception as exc:  # network / shape / parse — all non-fatal
