...
import { PLAZA_POS, getRealmPlaces } from "./placeRegistry";
...
function useVillageMaterials() {
  const mats = useMemo(() => {
    ...
    // Blueprint for locked buildings
    const blueprint = glow(
      new THREE.MeshStandardMaterial({
        color: hex(WORLD.botEye),
        emissive: hex(WORLD.botEye),
        wireframe: true,
        transparent: true,
        opacity: 0.35,
      }),
    ...
    return { stone, stoneHi, timber, roof, cobble, road, window, workWindow, lantern, glass, crystal, forgeTop, flame, flameTip, skillGlow, blueprint };
  }, []);
...
type BuildingGLTFProps = { url: string; mats: Mats; scale?: number; rotY?: number; locked?: boolean };

/** Loads a KayKit GLB, parses its materials by name, and intercepts them with the
 *  dynamic bioluminescent materials generated by useVillageMaterials. Uses drei's
 *  useGLTF (NOT a bare GLTFLoader): every village GLB is EXT_meshopt_compression-
 *  encoded, and useGLTF wires the meshopt decoder for us — a bare loader throws
 *  "setMeshoptDecoder must be called before loading compressed files". */
function BuildingGLTFInner({ url, mats, scale = 1, rotY = 0, locked }: BuildingGLTFProps) {
  ...
  const clone = useMemo(() => {
    ...
    c.traverse((o: any) => {
      if (o.isMesh) {
        ...
        if (locked) {
          o.material = mats.blueprint;
        } else if (matName.includes("Wood")) o.material = mats.timber;
...
/** Each building isolates its own GLB load behind a Suspense + ErrorBoundary, so a
 *  single missing or corrupt asset degrades THAT building to nothing instead of
 *  throwing past World3D's Suspense and collapsing the whole Grove to the 2D fallback. */
function BuildingGLTF(props: BuildingGLTFProps) {
  return (
    <ErrorBoundary
      fallback={null}
      onError={(e) => console.error(`[Village3D] failed to load ${props.url}:`, (e as Error)?.message)}
    >
      <Suspense fallback={null}>
        <BuildingGLTFInner {...props} />
      </Suspense>
    </ErrorBoundary>
  );
}
...
function Tavern({ mats, locked }: { mats: Mats; locked?: boolean }) {
  return (
    <group>
      <BuildingGLTF url="/models/village/Tavern.glb" mats={mats} scale={0.7} rotY={-Math.PI / 2} locked={locked} />
      {!locked && <Lantern mats={mats} x={1.2} z={1.4} h={1.3} />}
    </group>
  );
}

/** Forge/workshop: Uses Forge.glb */
function Forge({ mats, locked }: { mats: Mats; locked?: boolean }) {
  return (
    <group>
      <BuildingGLTF url="/models/village/Forge.glb" mats={mats} scale={0.7} rotY={-Math.PI / 2} locked={locked} />
...
function Greenhouse({ mats, locked }: { mats: Mats; locked?: boolean }) {
  return (
    <group>
      <BuildingGLTF url="/models/village/Greenhouse.glb" mats={mats} scale={0.65} rotY={0} locked={locked} />
...
/** Procedural stone well — Well.glb was never shipped, so we build the well from
 *  primitives (stone drum + rim + timber roof) rather than 404-ing on a missing GLB. */
function WellBody({ mats, locked, scale = 1 }: { mats: Mats; locked?: boolean; scale?: number }) {
  const stone = locked ? mats.blueprint : mats.stone;
  const timber = locked ? mats.blueprint : mats.timber;
  const roof = locked ? mats.blueprint : mats.roof;
  return (
    <group scale={scale}>
      {/* stone drum */}
      ...
      {/* two timber posts + a small pitched roof */}
      {[-0.5, 0.5].map((x) => (
        <mesh key={x} position={[x, 1.35, 0]} castShadow>
          <boxGeometry args={[0.1, 1.1, 0.1]} />
          <primitive object={timber} attach="material" />
        </mesh>
      ))}
...
function TasksWell({ mats, locked }: { mats: Mats; locked?: boolean }) {
  return (
    <group>
      <WellBody mats={mats} locked={locked} scale={1.2} />
...
function CalendarStone({ mats, locked }: { mats: Mats; locked?: boolean }) {
  return (
    <group>
      {/* Tall stone obelisk */}
      <mesh position-y={1.5} castShadow>
        <boxGeometry args={[0.6, 3.0, 0.6]} />
        <primitive object={locked ? mats.blueprint : mats.stone} attach="material" />
      </mesh>
...
function Archives({ mats, locked }: { mats: Mats; locked?: boolean }) {
  return (
    <group>
      {/* Mill.glb is used as a placeholder for the Archives */}
      <BuildingGLTF url="/models/village/Mill.glb" mats={mats} scale={0.7} rotY={0} locked={locked} />
...
function Building({ def, mats, activePlaces }: { def: BuildingDef; mats: Mats; activePlaces: string[] }) {
  const Body =
    def.kind === "tavern"
      ? Tavern
      : def.kind === "workshop"
...
export function Village3D({ reduced, skills }: { reduced: boolean; skills: Skill[] }) {
  ...
  const activePlaces = useMemo(() => getRealmPlaces(activeRealm).map((p: any) => p.id), [activeRealm]);
  ...
  return (
    <group ref={root}>
      ...
      <Roads mats={mats} />
      {BUILDING_DEFS.map((b) => (
        <Building key={b.id} def={b} mats={mats} activePlaces={activePlaces} />
      ))}
