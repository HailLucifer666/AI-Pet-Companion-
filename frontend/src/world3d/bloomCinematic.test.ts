...
describe("bloomFlash â€” cubic-out level-up flash", () => {
  ...
  it("is cubic-out (non-linear) â€” well below 0.5 by the midpoint", () => {
    const f = bloomFlash(700, 1400); // linear decay would be 0.5
    expect(f).toBeGreaterThan(0);
    expect(f).toBeLessThan(0.2);
  });

  it("clamps a negative since (clock skew) to peak â€” never a spurious dark gate", () => {
...
describe("bloomGateScale", () => {
  it("rest (no flash) â†’ no swell", () => {
    expect(bloomGateScale(0)).toBe(1);
  });

  it("full bloom â†’ 12% swell", () => {
