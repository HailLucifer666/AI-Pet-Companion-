/** flow â€” the drifting light-motes' motion, as pure math (no three, no GPU).
 ...
 *  Pure â†’ unit-tested in Node; deterministic so the field never reshuffles. */
 ...
 *  floating between `yMin` and `yMax`. Same seed â†’ identical field every launch. */
export function makeField(count: number, seed: number, radius = 13, yMin = 0.6, yMax = 6): MoteField {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const rad = Math.sqrt(r()) * radius; // sqrt â†’ even areal spread, not centre-clustered
    motes.push({
      bx: Math.cos(ang) * rad,
      bz: Math.sin(ang) * rad,
      by: yMin + r() * (yMax - yMin),
 ...
 *  into `out`. The drift is two offset sines per axis â†’ a smooth, non-repeating
