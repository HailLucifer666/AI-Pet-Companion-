...
describe("weatherFx", () => {
  it("clear â†’ barely any cloud, no rain, no dim", () => {
  ...
  it("rain â†’ it rains and the light dims", () => {
  ...
  it("storm â†’ heavy rain + lightning", () => {
  ...
  it("fog â†’ fog draws in tight", () => {
    expect(weatherFx("fog").fogScale).toBeLessThan(0.6);
  });

  it("snow â†’ snow flag set", () => {
    expect(weatherFx("snow").snow).toBe(true);
  });

  it("overcast â†’ heavy cloud, no precip", () => {
...
describe("fxFor", () => {
  it("unavailable weather â†’ clear fallback", () => {
    const fx = fxFor({ available: false });
    expect(fx.rain).toBe("none");
    expect(fx.dim).toBe(0);
  });

  it("available â†’ maps the category", () => {
