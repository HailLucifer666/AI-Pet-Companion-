/** widening.ts â€” The Widening: the world literally opens up as the companion
 *  matures. Driven by the REAL `pet.stage` signal (backend ladder.py, 1â€“4), each
 ...
 *  up â€” "it will grow into this." Pure + unit-tested; World3D/Atmosphere just read
 *  these targets. The stage-up *moment* itself reuses the cubic-out bloomFlash. */

export interface StageReveal {
  surveyDist: number; // camera max zoom-out â€” how much of the island you can take in
  fogFar: number; // how far the horizon fog reaches before dissolving the world
}

// Indexed by stage 1..4 (index 0 unused). Monotonic â€” the world only ever opens.
